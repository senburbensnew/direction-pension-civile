...
<div class="max-w-2xl mx-auto"
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-800">Ajouter un partenaire</h1>
            <p class="text-sm text-gray-500">Section "Nos Partenaires" de la page d'accueil</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.partenaires.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" required
                   placeholder="Ministere de l'Economie et des Finances"
        ...
        <div>
            ...
            <div
                ...
                @dragover.prevent
                @drop.prevent="const f=$event.dataTransfer.files[0]; if(f&&f.type.startsWith('image/')){$refs.logoInput.files=$event.dataTransfer.files;handleFile({target:{files:$event.dataTransfer.files}})}"
                class="border-2 border-dashed border-gray-300 rounded-xl p-5 text-center cursor-pointer hover:border-blue-400 transition-colors"
            >
                <template x-if="!preview">
                    <div>
                        <i class="fas fa-image text-2xl text-gray-300 mb-1"></i>
                        <p class="text-sm text-gray-500">Cliquez pour choisir un logo</p>
                        <p class="text-xs text-gray-400 mt-1">PNG, SVG, JPG — max 2 Mo</p>
                    </div>
                </template>
                <template x-if="preview">
                    <div>
                        <img :src="preview" class="max-h-24 mx-auto object-contain">
        ...
        {{-- Website --}}
        <div>
            <label for="website_url" class="block text-sm font-medium text-gray-700 mb-1">Site web (optionnel)</label>
