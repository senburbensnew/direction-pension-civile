...
@section('breadcrumb')
    <span class="text-gray-700 text-sm">Médiathèque</span>
@endsection

@section('content')
<div class="space-y-4" x-data="mediaAdmin()">
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">Aperçu</th>
            ...
            <tbody>
                @forelse($items as $item)
                    @php
                        $typeIcons = ['image' => 'fa-image text-green-600', 'video' => 'fa-video text-purple-600', 'audio' => 'fa-music text-yellow-600', 'document' => 'fa-file-pdf text-red-600'];
                    ...
                    <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($item->type === 'image' && $item->file_path)
                                <img src="{{ $item->fileUrl() }}" alt="{{ $item->title }}" class="h-10 w-16 object-cover rounded inline-block">
                            @elseif($item->file_path)
                                <a href="{{ $item->fileUrl() }}" target="_blank" class="text-blue-600 hover:underline text-xs"><i class="fas fa-download"></i> Fichier</a>
                            @elseif($item->url)
                                <a href="{{ $item->url }}" target="_blank" class="text-blue-600 hover:underline text-xs"><i class="fas fa-external-link-alt"></i> Lien</a>
                            @else
                                <span class="text-gray-300 text-xs">—</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{{ $item->created_at->format('d/m/Y') }}</td>
                        <td class="px-4 py-3 text-center">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('admin.mediatheque.toggle', $item) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="px-2 py-1 rounded text-xs font-medium {{ $item->published ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }}">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-gray-400">
                            <i class="fas fa-photo-film text-3xl mb-2 block"></i>
                            Aucun élément dans la médiathèque.
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6 space-y-4 z-10 max-h-[90vh] overflow-y-auto">
            ...
            <form :action="editing ? '/admin/mediatheque/' + form.id : '{{ route('admin.mediatheque.store') }}'"
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Fichier (image, vidéo, audio, PDF…)</label>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">URL externe (YouTube, SoundCloud…)</label>
