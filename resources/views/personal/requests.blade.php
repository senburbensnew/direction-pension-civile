                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        <a class="text-blue-600 hover:text-blue-900 mr-2"
                                            href="{{ route('personal.request.authenticated-user-request.show', $request->id) }}">
                                            Détails
                                        </a>
                                        @if ($request->status === 'pending')
                                            <button type="button"
                                                onclick="window.dispatchEvent(new CustomEvent('open-delete-modal', { detail: { url: '{{ route('demandes.destroy', $request->id) }}', title: '{{ addslashes($request->title ?? '') }}' } }))"
