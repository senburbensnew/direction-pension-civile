...
@if($label)
    <label for="{{ $inputId }}" class="block text-base font-medium text-gray-700 mb-1">
...
>
    <label for="{{ $inputId }}" class="flex items-center gap-4 p-4 cursor-pointer">
        ...
        <div class="flex-1 min-w-0">
            <p class="text-base font-medium text-navy">{{ $chooseLabel }}</p>
            <template x-if="files.length <= 1">
                <p x-text="fileLabel" class="text-base text-gray-600 truncate mt-0.5">{{ $noFileLabel }}</p>
            </template>
            <template x-if="files.length > 1">
                <ul class="mt-1 space-y-0.5">
                    <template x-for="file in files" :key="file.name">
                        <li class="flex items-center gap-1 text-base text-gray-700">
            ...
            <p class="text-base text-gray-600 mt-0.5">{{ $dropLabel }}</p>
        </div>
        <input
            x-ref="input"
            type="file"
...
@if($hint)
    <p class="mt-1 text-base text-gray-600">{{ $hint }}</p>
@endif
